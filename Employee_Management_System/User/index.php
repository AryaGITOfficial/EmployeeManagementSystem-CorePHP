...
if (isset($_POST['login'])) {
  ...
  $query = mysqli_query($con, "select * from users where  username='$uname' && password='$Password' ");
  $ret = mysqli_fetch_array($query);
  if ($ret > 0) {
    $_SESSION['aid'] = $ret['id'];
    switch ($ret['user_category']) {
      case "admin":
        header("location:admin.php?userid={$_SESSION['aid']}");
        break;
      case "manager":
        header("location:manager.php?userid={$_SESSION['aid']}");
        break;
      case "staff":
        header("location:staff.php?userid={$_SESSION['aid']}");
        break;
      default:
        header('location:no_user_category.php');
        break;
    }
  } else {
